...
async function main() {
  ...
  console.log(
    '✅ Configuración creada (C=5, pero 2 médicos requeridos por día)'
  );
  ...
  const medicos = await Promise.all([
    Factories.createMedico({
      nombre: 'Dr. Solitario',
      email: 'solitario@hospital.com',
    }),
    Factories.createMedico({
      nombre: 'Dra. Sobrecargada',
      email: 'sobrecargada@hospital.com',
    }),
  ]);
  console.log(
    `✅ ${medicos.length} médicos creados (muy posos para la demanda)`
  );
  ...
  console.log(
    '✅ Período de 5 días creado (con req de 2 medicos/dia = 10 turnos totales)'
  );
  ...
  console.log(
    `✅ ${disponibilidadData.length} registros de disponibilidad creados (Oferta insuficiente)`
  );
  ...
  console.log(
    '   - Resultado esperado: Infactible (Min-Cut detectará falta de cobertura)'
  );
}
