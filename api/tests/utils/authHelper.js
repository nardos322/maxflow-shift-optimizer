...
const AuthHelper = {
    ...
    async createUserAndGetToken(role = 'ADMIN', data = {}) {
        const email = data.email || `${role.toLowerCase()}${Math.floor(Math.random() * 10000)}@test.com`;
    ...
    async getAdminToken() {
        const { token } = await this.createUserAndGetToken('ADMIN', { email: 'admin@hospital.com', password: 'admin123' });
        return token;
    },
